...
scp -q -o ConnectTimeout=8 "$WCRC/drive_fast.py" "$SSH:~/drive_fast.py" || die "코드 업로드 실패"
grn "[3/4] 코드 업로드 OK"

# 4. 출발 전 점검
echo "[4/4] 출발 전 점검..."
echo "────────────────────────────────────"
ssh -o ConnectTimeout=8 "$SSH" "cd ~ && $ENVS python3 drive_fast.py check" 2>&1 | grep -v '^\['
echo "────────────────────────────────────"
...
if [ "$1" = auto ]; then
  ...
  echo
  ssh -t "$SSH" "cd ~ && $ENVS python3 drive_fast.py run now"
else
  ...
  echo
  ssh -t "$SSH" "cd ~ && $ENVS python3 drive_fast.py run"
fi
